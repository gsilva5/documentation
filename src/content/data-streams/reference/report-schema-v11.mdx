...
<Aside type="caution" title="Deprecated: lastTradedPrice">
  The `lastTradedPrice` field is deprecated. New integrations must not depend on this field, and existing integrations
  should remove any production dependency by **October 1, 2026**.

  The field will remain in the RWA Advanced (v11) schema and may continue to be populated where source data is
  available. Beginning October 1, 2026, however, it will no longer be monitored for availability, accuracy, or data
  quality. Any continued use is unsupported and at the user's own risk.
</Aside>
...
- **IMPORTANT**: The `lastSeenTimestampNs` field reflects the timestamp of the last update for the `mid` price only. Do not assume this timestamp applies to `bid`, `ask`, `bidVolume`, or `askVolume`.
