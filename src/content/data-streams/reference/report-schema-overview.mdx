...
- **[RWA Advanced (v11)](/data-streams/reference/report-schema-v11)** — provides bid and ask prices, volume depth, nanosecond-precision timestamps, and expanded market-status values.
...
The [RWA Advanced (v11)](/data-streams/reference/report-schema-v11) report schema offers liquidity-weighted mid (`mid`), consensus bid and ask prices (`bid`, `ask`), resting-book depth (`bidVolume`, `askVolume`), nanosecond-precision freshness (`lastSeenTimestampNs`), and expanded market status (`marketStatus`). The schema also contains `lastTradedPrice`, which is deprecated and should not be used in new integrations or as a production input.
