...
The schema also retains the deprecated `lastTradedPrice` field for compatibility. New integrations must not depend on this field, and existing integrations should remove any production dependency by **October 1, 2026**. See the [RWA Advanced v11 schema documentation](/data-streams/reference/report-schema-v11) for the full deprecation notice.

<Aside type="caution" title="Important: Timestamp Scope">
  The `lastSeenTimestampNs` field reflects the timestamp of the last update for the `mid` price only. It does not apply
  to `bid`, `ask`, `bidVolume`, or `askVolume`.
</Aside>
